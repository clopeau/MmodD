...
function p0_3d_slice(%v,%x,%y,%z,cbar,theta,alpha,leg,flag,ebox)
    %th=evstr(%v.geo); 
    opts=[]
    ...
    if ~exists('%x','local');
      ...
      %y=(xyzminmax(2,2)-xyzminmax(2,1))/2;
      %z=(xyzminmax(3,2)-xyzminmax(3,1))/2;
   end
   ...
   for dim=1:3
	 ...
	 if length(%ind_loc)>0
	   ...
	   %YY=[%YY , %Y'];
	   %ZZ=[%ZZ , %Z'];
	   VCell=[VCell, %v.Cell(%ind_loc)'];
	 end
       end
       if %XX<>[]
	 VCell=round(coulmax*(VCell-zminmax(1))/(zminmax(2)-zminmax(1)))
	 execstr('plot3d(%XX,%YY,list(%ZZ,VCell),'+strcat(opts,',')+')');
       end
       //============================================
