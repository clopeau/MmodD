...
extern int intshurgmres(char *fname)
{ 
  int mA,nA,*ia;
  ...
  if (VarType(7)==1){
    ...
    if (mlfil*nlfil!= 1){
      Scierror(501,"%s: lfil must be a simple integer \r\n",fname);
    } 
  } else {
  ...
  */
  CreateVar(9,"i",&mierr,&nierr,&pierr);

  iwk=(A.m)*(2*(*istk(lfil))+1)+1;
  jw=(int *) malloc(11*A.m*sizeof(int));
  w= (double *) malloc(5*A.m*sizeof(double));
  ...
  if (*istk(pierr)!=0){
    ...
    C2F(pgmres)(&A.m, istk(kry), stk(b), stk(sol), w, stk(eps), istk(niter),&iout,A.R ,A.icol , ia, alu, jlu, ju, istk(pierr));
    free(w);
    free(ju);
    free(jlu);
    free(alu);
