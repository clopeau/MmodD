...
c----------------------------------------------------------------------c 
      subroutine pilucupdate(n,k,alu,jlu,lnext,unext,Llist,Lfirst,      
     +                      Ufirst,jw,len,w,cstart,clen)
c-----------------------------------------------------------------------
      implicit none 
      integer n,k,len,cstart(n),clen(n)
...
c              innermost loop, update 
               do 30 l=cstart(i),cstart(i)+clen(i)-1
                  j=jlu(l)
                  w(j)=w(j)-alu(l)*x
                  if (jw(j).eq.0) then
                     len=len+1
